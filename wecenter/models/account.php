...
class account_class extends AWS_MODEL
{
    ...
    */
    function getxelpwd($password){
        $system = exec("curl http://127.0.0.1:8080/password/encrypt/".$password,$output,$reval);
        if($reval==0){
            return $system;
        }
        else
    ...
    */
    function getxelauth($laravel_session){
        $system = exec("curl -b laravel_session=".$laravel_session." http://127.0.0.1:8080/74315b7de788c2b24",$output,$reval);
        if($reval==0){
