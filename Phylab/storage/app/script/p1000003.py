...
output:
   ...
   y2_aver = np.average(y2),
   xy_aver = np.average(xy),
   i = i,#第二个表格的次数xi
   D2 = D2,#第二个表格的yi
   i2 = i2,#第二个表格的xi的平方
   D4 = D4,#第二个表格的yi的平方
   ...
   D2_aver = np.average(D2),
   i2_aver = np.average(i2),
   D4_aver = np.average(D4),
   D2i_aver = np.average(D2i),
	delta_lambda = delta_lambda,#单位nm
	u_delta_lambda = u_delta_lambda,#单位nm
	delta = delta,#相对误差
	r = r,#第一问的相关系数
	b = b,#第一问的b，单位m
	bb = bb,#证明的b，单位m
...
def readXml2130014(root, source):
...
def handler(XML):
    file_object = open(texdir + "Handle1000003.tex","r")
    source = file_object.read().decode('utf-8', 'ignore')
    file_object.close()
    return readXml2130014(XML, source)
      
if __name__ == '__main__':
    scriptdir = 'D:/Apache24/htdocs/PhyLabs/Phylab/storage/app/script/'
    texdir = scriptdir + 'tex/'
    dom = xml.dom.minidom.parse(scriptdir + 'test/1000003test/1000003.xml')
