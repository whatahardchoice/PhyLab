...
${i}_{min}$的不确定度：
$$u({i}_{min}) = \sqrt{u^2_a({i}_{min})+u^2_b({\delta}_{min})} = %% U_I %%$$
...
$$u(n_2) = \frac{1}{n}\frac{cosA+sin\bar{i_{min}}}{sin^2A}[cos\bar{i_{min}u(\bar{i_{min})^2+(\frac{1+cosAsin\bar{i_{min}}}{sinA}u(A))^{\frac{1}{2}}]}]^{\frac{1}{2}} = %%U_N %%$$
相对不确定度：
$$\displaystyle\frac{u(n_2)}{n_2} = %% RE_U %%$$

最终结果:
$${n_1}{\pm}u({n_1}) = %% RESULT_N %% {\pm} %% RESULT_U_N %% $$